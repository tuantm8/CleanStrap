...
$LoggedinUserAvatar = cs_get_avatar(qa_get_logged_in_handle(), 30, false);
			if (!empty($LoggedinUserAvatar)) {
				echo '<img src="' . $LoggedinUserAvatar . '" />';
			}
			// in case there is no Avatar image and theme doesn't use a default avatar
			else {
...
$this->output(ob_get_clean());
	...
	function q_view_main($q_view) {
		...
		$this->favorite();
		$this->voting($q_view);
		$this->output(htmlentities($q_view['raw']['title'], ENT_QUOTES, ENT_HTML5, 'UTF-8'));
		$this->output('</h2>');
	...
	function main_part($key, $part) {
		...
		if ($partdiv) {
			$this->output('<div class="qa-part-' . strtr($key, '_', '-') . '">');
		}
